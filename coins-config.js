...
//
// v5.60 (2026-08-29): FEE CORRECTION — feeEst was ~12x too high on every HIP-3 asset.
//   Measured from .fills-cache.json (899 fills), % per side, stable in EVERY month:
//     BTC        0.045  (config 0.05  — correct, unchanged)
//     xyz:SP500  0.0086 (config 0.10  -> 0.01)
//     xyz:CL     0.0085 (config 0.12  -> 0.01)
//     xyz:XYZ100 0.0084 (config 0.10  -> 0.01)
//     xyz:GOLD   0.087  <- the real builder fee; the 0.1x values were copied from here
//   Why it mattered: calcRR() (signals.js:528) adds round-trip fees to risk and subtracts
//   them from reward, then coinMinRR gates on the result. At each asset's minStopPct a
//   clean 2R setup scored RR 1.10 / 0.99 / 1.10 against a 1.2 gate — BLOCKED. At the true
//   fee the identical setup scores RR 1.81 and passes. So the retest engine has been
//   rejecting its TIGHTEST-STOP, BEST-R:R setups on all three HIP-3 assets and admitting
//   only wider-stop (worse) ones — a plausible contributor to the edge decay behind v5.59.
//   0.01 (not 0.0085) is deliberate: ~18% conservative, covers fee drift.
//   No live behaviour change while retest is paused; this takes effect on re-enable.
//   Re-run counterfactual.js against the corrected gate before restoring equityPct.
//   Provenance: reversal-strategy-2026-08-29.md §6.
//
...
// v5.49 (2026-07-15): SAME-DIRECTION RE-ENTRY COOLDOWN — cooldownMin (minutes).
//   After a position on an asset closes, the bot will NOT open a NEW position in the
//   SAME direction on that asset until cooldownMin minutes have elapsed. Opposite-
//   direction (reversal) signals are unaffected. Set 0 to disable per asset.
//   Rationale: 2026-07-14 crude — RT1 long banked +$69.52 at 09:45, then the bot
//   re-entered the SAME long 19 min later (10:04) ~1.2% higher, near the top of the
//   move; Tomaž closed it manually to dodge the SL (+$2.62 scrape). 90d scan: this
//   "re-enter same asset within 30 min" pattern occurred exactly ONCE (that trade),
//   and normal spacing's next-fastest same-asset re-entry was >1h out — so a 45-min
//   cooldown blocks this failure mode and nothing else (cannot dent the edge; it is
//   cheap insurance, not a proven dollar-saver). Time-based, so it is NOT the rejected
//   price-based minImpulseRemaining filter. Gate lives in bot.js maybeAutoTrade(),
//   keyed off loadClosedTrades() (most recent same-coin, same-side close ts).
const COINS = {
  bitcoin:     { id:'bitcoin',     label:'BTC',    apiSym:'BTCUSDT',    asset:'BTC',        exchange:'binance',     minRR: 1.0, feeEst: 0.05, minStopPct: 0.010, equityPct: 0, cooldownMin: 45, isHIP3: false, minBreakBodyPct: 0.008, closeExitR: 1.0, hardSlMult: 1.3, longOnly: true }, // v5.59 (2026-08-29): RETEST PAUSED — equityPct 3.00→0 (canonical off-switch, bot.js L1738: signal still paper-logged, no live trade). Manual pause by Tomaž after a −$88 (−7.4%) day; 7d retest ≈ −$98 vs playbook ~flat. Restore to 3.00 to re-enable. Playbook (crude/xyz100 TBL) is unaffected — sizes off PLAYBOOK_RISK_PCT. // v5.57 (2026-08-24): longOnly TRUE — extend the v5.52 SP500 short-block to all retest assets. 60d entry-cloid-paired review: RETEST shorts lost ~$185 across every asset (SP500 −$95/6, BTC −$63/4, crude −$16/19, xyz100 −$11/20; 15W/49 = 31%) while retest longs ran ~breakeven-positive and playbook (longs-only) made +$96. Blocking retest shorts flips the 60d book from −$117 to ~+$68 pro-forma. Re-validate via .missed_signals.json blocked-shorts log at ~4 weeks. See daily review 2026-08-24. NOTE: PLAYBOOK is already longs-only and unaffected. // v5.43 (2026-07-09): 1.93→3.00 — re-normalized to 3% risk-per-trade after Jul-7 deposit reset equity to ~$2,868. 3.00 × minStop 1.0% = 3% design risk. Drops the v5.40 ×1.93 dollar-hold inflator (no longer meaningful at 5.3× equity). // v5.37: wick immunity — exit on 15m CLOSE through level SL, hard stop 1.3R
  sp500:       { id:'sp500',       label:'S&P500', apiSym:'xyz:SP500',  asset:'xyz:SP500',  exchange:'hyperliquid', minRR: 1.2, feeEst: 0.01, minStopPct: 0.005, equityPct: 0, cooldownMin: 45, isHIP3: true,  breakDistFloorPct: 0.0008, minBreakCloses: 1, closeExitR: 0.5, longOnly: true }, // v5.60 (2026-08-29): FEE CORRECTION — feeEst 0.10→0.01. Measured from .fills-cache.json (899 fills): xyz:SP500 actually pays 0.0086%/side, stable every month (Apr–Jul). The 0.1x values were copied from GOLD, the only asset that genuinely paid the ~0.087% builder fee. calcRR() (signals.js:528) adds round-trip fees to risk and subtracts them from reward, so the stale value made a clean 2R setup at minStopPct score RR 1.10 against the 1.2 minRR gate — BLOCKED. At the true fee the same setup scores RR 1.81 and passes. The retest engine has been rejecting its tightest-stop, best-R:R setups on all three HIP-3 assets and admitting only wider-stop ones; likely contributor to the edge decay behind the v5.59 pause. See reversal-strategy-2026-08-29.md §6. BTC unchanged (0.05 vs 0.045 actual — correct). // v5.59 (2026-08-29): RETEST PAUSED — equityPct 6.00→0 (was already effectively idle: SP500 long-only + no fills since Jun 25). Restore to 6.00 to re-enable. // v5.52 (2026-07-25): longOnly TRUE — block SHORT auto-entries. SP500 shorts are the book's single biggest drag: 90d 10 RTs, 30% win, PF 0.23, −$153 (every month negative, −$129 in Jul); ungated retest core −6.1R/7 over 65d. Longs breakeven-positive over the same window, so this closes the short-side asymmetry (shorts were never quality-gated). XYZ100 kept two-sided per Tomaž (Jul-25 call). Re-validate via .missed_signals.json blocked-shorts log after ~4 weeks. See entry-strategy-review-2026-07-23.md. // v5.43 (2026-07-09): 3.86→6.00 — re-normalized to 3% risk-per-trade after Jul-7 deposit (equity ~$2,868). 6.00 × minStop 0.5% = 3% design risk. Proven edge stays at full core weight. // v5.37: structure close-exit — 15m close >=0.5R against entry exits all
  crude:       { id:'crude',       label:'CRUDE',  apiSym:'xyz:CL',     asset:'xyz:CL',     exchange:'hyperliquid', minRR: 1.2, feeEst: 0.01, minStopPct: 0.005, equityPct: 0, cooldownMin: 45, isHIP3: true,  breakDistFloorPct: 0.0008, minBreakCloses: 1, closeExitR: 0.5, longOnly: true }, // v5.60 (2026-08-29): FEE CORRECTION — feeEst 0.12→0.01. Measured from .fills-cache.json (899 fills): xyz:CL actually pays 0.0085%/side, stable every month (May–Aug). The 0.1x values were copied from GOLD, the only asset that genuinely paid the ~0.087% builder fee. calcRR() (signals.js:528) adds round-trip fees to risk and subtracts them from reward, so the stale value made a clean 2R setup at minStopPct score RR 0.99 against the 1.2 minRR gate — BLOCKED. At the true fee the same setup scores RR 1.81 and passes. The retest engine has been rejecting its tightest-stop, best-R:R setups on all three HIP-3 assets and admitting only wider-stop ones; likely contributor to the edge decay behind the v5.59 pause. See reversal-strategy-2026-08-29.md §6. BTC unchanged (0.05 vs 0.045 actual — correct). // v5.59 (2026-08-29): RETEST PAUSED — equityPct 1.20→0. Restore to 1.20 to re-enable (NOT higher — crude edge decayed to ~0.99 PF). PLAYBOOK crude TBL legs unaffected (flat PLAYBOOK_RISK_PCT). // v5.57 (2026-08-24): longOnly TRUE — retest crude shorts −$16/19 over 60d (2W); retest crude longs ~breakeven. See v5.57 note on bitcoin. PLAYBOOK crude legs are already longs-only and unaffected. // v5.53 (2026-07-27): 2.40→1.20 — TRIM (reverses the v5.44 conviction bump). The v5.44 premise decayed: 90d PF fell to ~0.99 (breakeven ex-outlier), edge no longer justifies half-core weight (crude-edge-decay 2026-07-21). Back to the pre-bump 1.20 = 0.6% risk until crude re-clears validation (PF>1.2 over 20+ RTs). NOTE: this is the RETEST cap only — the PLAYBOOK crude legs (TBL/DBL) size off flat PLAYBOOK_RISK_PCT and are unaffected. // v5.44 (2026-07-10): 1.20→2.40 CONVICTION BUMP (90d 37 RTs, PF 1.38, +$11.52) — now reverted. // closeExitR 0.5 (matches SP500): structure close-exit for the squeeze failure mode; losers are short counter-squeezes ~1.5%.
  xyz100:      { id:'xyz100',      label:'XYZ100', apiSym:'xyz:XYZ100', asset:'xyz:XYZ100', exchange:'hyperliquid', minRR: 1.2, feeEst: 0.01, minStopPct: 0.005, equityPct: 0, cooldownMin: 45, isHIP3: true,  breakDistFloorPct: 0.0008, minBreakCloses: 1, longOnly: true }, // v5.60 (2026-08-29): FEE CORRECTION — feeEst 0.10→0.01. Measured from .fills-cache.json (899 fills): xyz:XYZ100 actually pays 0.0084%/side, stable every month (Jul–Aug). The 0.1x values were copied from GOLD, the only asset that genuinely paid the ~0.087% builder fee. calcRR() (signals.js:528) adds round-trip fees to risk and subtracts them from reward, so the stale value made a clean 2R setup at minStopPct score RR 1.10 against the 1.2 minRR gate — BLOCKED. At the true fee the same setup scores RR 1.81 and passes. The retest engine has been rejecting its tightest-stop, best-R:R setups on all three HIP-3 assets and admitting only wider-stop ones; likely contributor to the edge decay behind the v5.59 pause. See reversal-strategy-2026-08-29.md §6. BTC unchanged (0.05 vs 0.045 actual — correct). // v5.59 (2026-08-29): RETEST PAUSED — equityPct 3.00→0. Restore to 3.00 to re-enable. PLAYBOOK xyz100 TBL unaffected (flat PLAYBOOK_RISK_PCT). // v5.58 (2026-08-24): 1.50→3.00 CONVICTION BUMP to full core weight (3.00 × minStop 0.5% = 1.5% design risk). Cleared crude-protocol at n=20 closed bot RTs: PF 1.73, 50% win, +$124 closedPnl (+$112 after fees), all opens cloid 0x444d5300 (zero manual). Weekly-validation call 2026-08-24. CAVEAT: equity had fallen to ~$1,303 at bump time and equity-base sizing instability is live (Aug opens ran $6–7.5k notional); watch realized notional vs target. // v5.57 (2026-08-24): longOnly TRUE — reverses the Jul-25 two-sided call. Retest xyz100 shorts −$11/20 over 60d (9W/20 but net-negative) vs retest longs +$34/19; playbook xyz100 TBL (longs) +$102/6 is the real earner. See v5.57 note on bitcoin. // v5.45 (2026-07-10): NEW ASSET — Nasdaq-100 perp (xyz HIP-3, $466M/24h — 2nd deepest on dex, 30× lev). Study counterfactual-xyz100-2026-07-10.md: raw Retest beat SP500 at EVERY TF on the identical window (15m PF 0.47 vs 0.33, 1H 0.39 vs 0.12, 4H 0.33 vs 0.22), 2.5× signal frequency. Config = SP500 index template. equityPct 1.50 = 0.75% risk (¼ core) — BELOW half-of-comparable because corr to SP500 is 0.89: this is concentration in the proven index edge, not diversification. NO closeExitR until it has its own counterfactual on live RTs. Validate at 15–20 closed RTs (crude protocol) before any bump toward 3.00.
};
