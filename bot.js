...
async function maybePlaybookTrade(coinId, h1C, price) {
  ...
  for (const sig of sigs) {
    ...
    try {
      ...
      // v5.56 FIX: the playbook flat bracket has NO management-loop fallback (unlike the
      // retest engine's trail/BE/close-exit), so the TP MUST fill on trigger. The old
      // limit-trigger + 2% cushion rested the sell 2% ABOVE the TP (28871.26 → 29448.69 on
      // the 2026-08-03 xyz100 trade); the trigger fired at 28871 but the limit sat ~2%
      // above the market (session high 28942, +0.25%) and never filled — the +2R winner
      // round-tripped until closed manually. Use a market trigger like the SL above; the
      // ~few-bp taker fee is trivial vs letting a +2R winner decay to a −1R stop.
      const tpRes = await HL.placeOrder(asset, !isBuy, size, isBuy ? sig.tp * 0.98 : sig.tp * 1.02,
        { trigger: { isMarket: true, triggerPx: sig.tp, tpsl: 'tp' } }, true);
...
const BOT_VERSION    = 'v5.60'; // v5.60 (2026-08-29): FEE CORRECTION — coins-config feeEst for xyz:SP500 0.10→0.01, xyz:CL 0.12→0.01, xyz:XYZ100 0.10→0.01 (+ same in dms-v53.html). Measured from .fills-cache.json (899 fills): all three pay ~0.0085%/side and have every month since listing; the 0.1x values were copied from GOLD, the only asset that really paid the ~0.087% builder fee. calcRR() (signals.js:528) inflates risk / deflates reward by feeEst before the coinMinRR gate, so a clean 2R setup at minStopPct scored RR 0.99–1.10 against a 1.2 gate and was BLOCKED; at the true fee it scores 1.81 and passes. Retest has been rejecting its tightest-stop best-R:R setups on all three HIP-3 assets. No behaviour change while retest is paused (equityPct 0) — takes effect on re-enable. BTC unchanged. See reversal-strategy-2026-08-29.md §6. ALSO in v5.60: dms-v53.html equityPct BTC 3.00/SP500 6.00/crude 1.20/xyz100 3.00 → 0, syncing the v5.59 retest pause into the dashboard's own COINS object — v5.59 changed coins-config.js only, and the dashboard app has its own auto-trade path that has produced live fills before (ghost-instance incident 2026-07-09), so retest was not actually paused with that tab open. Restore BOTH files together when re-enabling. // v5.59 (2026-08-29): RETEST ENGINE PAUSED — all four retest assets equityPct→0 in coins-config.js (canonical off-switch; signals still paper-logged, no live trades). Manual pause by Tomaž after −$88/−7.4% day; 7d retest ≈ −$98 vs playbook ~flat. PLAYBOOK (crude/xyz100 TBL) stays LIVE — sizes off PLAYBOOK_RISK_PCT, independent of equityPct. Reverse by restoring equityPct (BTC 3.00 / SP500 6.00 / crude 1.20 / xyz100 3.00). // v5.58 (2026-08-24): XYZ100 equityPct 1.50→3.00 conviction bump (coins-config.js + dashboard) — cleared crude-protocol at n=20 RTs (PF 1.73, +$124). v5.57 (2026-08-24): retest longOnly extended to BTC/CRUDE/XYZ100. // v5.56 (2026-08-04): PLAYBOOK TP FIX — HIP-3 playbook TP was a limit trigger with a 2% cushion resting the sell 2% ABOVE the target, so it never filled (flat bracket has no management-loop fallback); winners round-tripped to SL or manual close. TP is now a MARKET trigger like the SL. Affects both crude+xyz100 TBL. See bot.js ~L3508. // v5.55 (2026-08-03): BE_TRIGGER_R reverted 1.25 → 1.0 after weekly re-validation (40 new RTs; BE@1.0 +$15.21 vs BE@1.25 −$23.45 on fresh slice, and wins full 90d sample). Reverses the v5.36 bump. See revalidation-2026-08-03.md. // v5.54 (2026-07-27): PLAYBOOK HARDENING after live audit — DBL config-disabled (0/4 start, avg −1.49R vs −1.0R design: weekend/reopen stop slippage is structural, worst −2.26R at Sun CME reopen; crude regime flipped down and DBL buys falling knives); CME-closed guard blocks NEW playbook entries Fri 20:45→Sun 22:15 UTC; auto-bench min trades 10→6. TBL untouched (+0.99R avg live, on-script). See playbook.js PB_COINS comment. // v5.53 (2026-07-27): CRUDE TRIM — retest equityPct 2.40→1.20 (0.6% risk) in coins-config.js; reverses the v5.44 conviction bump after 90d PF decayed to ~0.99 breakeven (crude-edge-decay 2026-07-21). PLAYBOOK crude legs unaffected (flat PLAYBOOK_RISK_PCT). No logic change. // v5.52 (2026-07-25): SP500 LONG-ONLY — block SHORT auto-entries on longOnly assets (SP500) in maybeAutoTrade; shorts logged to .missed_signals.json for monthly audit. Retest short-side was the book's biggest drag (SP500 shorts 90d PF 0.23 / −$153). XYZ100 kept two-sided. See coins-config.js longOnly + entry-strategy-review-2026-07-23.md. // v5.51 (2026-07-23): PLAYBOOK engine — TBL (crude+xyz100) + DBL (crude) at PLAYBOOK_RISK_PCT (2%), closed-1H-bar detection, flat bracket SL 1×ATR / TP 2-4×ATR, auto-bench at −8R/20, kill via PLAYBOOK=off. See playbook.js + trade-playbook-2026-07-23.md.
